...
const CircleNode = ({ data, selected, id }: NodeProps<CircleNodeData>) => {
  const [isEditing, setIsEditing] = useState(false);
  const [label, setLabel] = useState(data.label || 'Circle Node');
  ...
  return (
    <div 
      className={`
        w-24 h-24 bg-node-bg border-2 rounded-full shadow-medium
